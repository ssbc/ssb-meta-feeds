...
test('messages', (t) => {
  ...
  t.test('add a feed to metafeed', (t) => {
  ...
  t.test('tombstone a feed in a metafeed', (t) => {
  ...
  t.test('second tombstone', (t) => {
    const opts = messages.optsForAddDerived(
      metafeedKeys,
      'main',
      seed,
      'classic'
  ...
  t.test('metafeed announce', (t) => {
    messages.optsForAnnounce(metafeedKeys, mainKey, (err, opts) => {
      ...
      t.ok(
        ssbKeys.verifyObj(metafeedKeys, opts.content),
...
test('recps', (t) => {
  ...
  sbot.box2.addKeypair(metafeedKeys)

  const opts = sbot.metafeeds.messages.optsForAddExisting(
    metafeedKeys,
    'main',
    mainKey,
    { recps: [mainKey.id] }
  ...
  sbot.db.create(opts, (err, encryptedKVT) => {
    t.error(err, 'no error')
    t.true(encryptedKVT.value.content.endsWith('.box2'), 'box2 encoded')
    sbot.db.get(encryptedKVT.key, (err, decryptedMsgVal) => {
      t.error(err, 'no error')
